...
# google.colab 用のlibrary, 特に2つ目はgoogle drive のファイルをコードで読み取るときに必須(driveをマウントする)
...
class CovarianceSelection:
    ...
    def init_matrix(self): 
        ...
        display(pd.DataFrame(self.pcor_mat(self.COR_mat), index=self.rc_name, columns=[self.rc_name]))
        print('\n')
    ...
    def check_ind(self, cnt, COV_mat, PCOR_mat, restore_flg=None):
        ...
        display(pd.DataFrame(pcor_mat, index=self.rc_name, columns=[self.rc_name]))
        print('\n')

        print('> estimated correlation matrix')
        display(pd.DataFrame(self.cor_mat(cov_mat), index=self.rc_name, columns=[self.rc_name]))
        print('\n')
    ...
    def do_covselection(self):
        ...
        while True:
            ...
            print('> estimated partial correlation matrix')
            display(pd.DataFrame(est_PCORmat, index=self.rc_name, columns=[self.rc_name]))
            print('\n')

            print('> estimated correlation matrix')
            display(pd.DataFrame(est_CORmat, index=self.rc_name, columns=[self.rc_name]))
            print('\n')
            ...
            #display(pd.DataFrame(self.residual, index=self.rc_name, columns=[self.rc_name]))
            print('\n')
...
def main():
    # 使用するデータのファイル名
    file_name = '/content/drive/My Drive/Colab Notebooks/pentathlon_2019_series/csv_files/modified_final_men_points_2019-world.csv'
    ...
    # 計算結果を保存するときのファイル名
    json_file_name = '/content/drive/My Drive/Colab Notebooks/pentathlon_2019_series/model-2019-men-modified/result.json'
    # 計算結果をjsonファイルに保存する
